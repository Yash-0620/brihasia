...
@app.route('/')
def serve_index():
    return send_from_directory('.', 'index_why.html')


# Serve the emotional assessment page
@app.route('/emotional-assessment.html')
def serve_emotional_assessment():
    return send_from_directory('.', 'emotional-assessment.html')
...
# New route to generate emotional visualization image
@app.route('/generate-emotion-image', methods=['POST'])
def generate_emotion_image():
    try:
        ...
        # For a real implementation, you would use an image generation API like:
        # 1. Stable Diffusion API (free options available)
        # 2. DALL-E API
        # 3. Midjourney API (if available)

        # Since we don't have a direct image generation API, we'll create
        # a abstract visualization based on the emotional content
